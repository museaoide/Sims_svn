screpD <- function(pgw=rep(0,times=nx*ny-nx*(nx-1)/2-ny),nx=8,ny=16,U=function(a,b){log(ifelse(a <= 0,1e-100,a))+log(ifelse(b-a <= 0,1e-100,b-a))},xrange=c(0,1),wrange=c(0,1),gg=function(b)b*(1-b), lambda=1)
  {
    ...
    ## gg:     marginal pdf of w; a function proportional to the density that will generate weights on wrange.
    ...
    k <- sum(pdf * log(ifelse(pdf>0,pdf,1))) - sum(pc * log(ifelse(pc>0,pc,1))) - sum(ggw * log(ifelse(ggw>0,ggw,1)))
    obf <- EU - lambda * k
