setUpScrep <- function(crange=c(0.,0.5), nc,  pw, ww, U=function(x,y) { log(pmax(x, .Machine$double.eps*2)) + log(pmax(y-x, .Machine$double.eps*2))}, kappa=1,
  ...
  ## Notes 6/1/2007:  Still need to fix derivative.  Seems possibly to be working, but numerically rough.
  ## ----------------FIX ABOVE---------------------
  ...
  dobjdpdc <- function(dc=rep(.1,9), pc=rep(.111,8)) {
    ...
    dobjdcv <- diag(pc) %*% DcU %*% h - lambda * ( apply(plogp(peuh), 1, sum)  - pdeuh %*% log(apply(peuh,2,sum)) )
