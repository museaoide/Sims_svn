setUpScrep <- function(crange=c(0.,0.5), pw, ww,
  ...
  screpDC <- function(dc=rep(.1,9), pc=rep(.111,8), alpha=1) {
    ...
    dhdalpha <- mul.tensor(dhdpwhat, "iw", dpwhatdalpha)
    ## /deriv--------------
    ...
    dpdfdpc <- diagmul.tensor(h, "iw", dpeaUdpc) #compacted
    dpdfdpc <- dpdfdpc + diagmul.tensor(dhdpc, "iw", peaU) #compacted
    ## for (i in 1:nc) dpdfdpc[ic=i,iw=1:nw , idc=1:nc] <- eaUh[ic=i, iw=1:nw]
    ## dpdfdpc <- dpdfdpc + mul.tensor(dpdfdh, "idw", dhdpc, "iw")
    dpdfdcc <- diagmul.tensor(dhdcc, "iw", peaU) + diagmul.tensor(h, "iw", dpeaUdcc)
    ...
    EU <- sum(pdf * Umat)
    ## deriv-----------------
    dEUdcc <- margin(diagmul.tensor(dpdfdcc, names(Umat), Umat), names(Umat))
    dEUdpc <- margin(diagmul.tensor(dpdfdpc, names(Umat), Umat), names(Umat))
    dEUdalpha <- margin(diagmul.tensor(dpdfdalpha, names(Umat), Umat), names(Umat))
    ## /deriv------------------
    ...
    ## deriv-------------------------
    dkdcc <- margin(diagmul.tensor(dpdfdcc, names(pdf), log(pdf) ) + dpdfdcc, names(pdf))
    dkdpc <- margin(diagmul.tensor(dpdfdpc, names(pdf), log(pdf) ) + dpdfdpc, names(pdf)) - log(pc)[[ic=~idp]] - one.tensor(c(idp=nc))
    dkdalpha <- margin(diagmul.tensor(dpdfdalpha, names(pdf), log(pdf) ) + dpdfdalpha, names(pdf))
    ## /deriv--------------------
    ## obf <- EU - lambda * k
    ...
    ## deriv-------------------------------
    dobfdcc <- dEUdcc - PNLTY * (mul.tensor(2 * (lbp / zeroc -1) * (-lbp / zeroc^2), "izc", dzerocdcc, "izc") + 10 * (k > kappa) * 2 * (k - kappa) * dkdcc)
    dobfdpc <- dEUdpc - PNLTY * (mul.tensor(2 * (lbp / zerop -1) * (-lbp / zerop^2), "izp", dzeropdp,  "izp") + 10 * (k > kappa) * 2  * (k - kappa) * dkdpc)
    dobfdalpha <- dEUdalpha - PNLTY * (10 * (k > kappa) * 2 * (k - kappa) * dkdalplha)
    ## /deriv---------------------------------
    return(list(obf=obf, EU=EU, cc=cc, pc=pc, pdf=pdf, info=k/log(2), dobfdcc, dobfdpc, dobfdalpha))
  }
