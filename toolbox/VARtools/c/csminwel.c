...
#define STRLEN 192
#define INDXNUMGRAD 2         //Index for choosing the numerical gradient.  1, forward difference; 2, central difference.
      ...
      *fcount += fc;
      ...
      if (*retcodeh && *retcodeh != 1)
         if (++cnt_n_badcases >= MAX_NUM_BADCASES) {
            ...
            #ifdef VERBOSE_WARNINGS
            printf("Hessian is reset to the initial value because the maximum number of bad cases, %d, is reached!", MAX_NUM_BADCASES);
            #endif
         }
...
static void csminit(double *fhat, double *xhat, int *fcount, int *retcode,
   ...
   memcpy(xhat, x0, n*sizeof(double));    //Iskander's original code does not have this line, which is a major bug.
   *fhat = f0;
   *fcount = 0;
   *retcode = 0;
