 ...
 *       to display all output, uncomment both VERBOSE_WARNINGS and VERBOSE_DETOUPUT
 *       to display only warnings without output, uncomment VERBOSE_WARNINGS
 *       to display no ouput, comment both VERBOSE_DETOUPUT and VERBOSE_WARNINGS
 *
 ...
 * Copyright 2002 Federal Reserve Bank of Atlanta
 *=======================================================
 * Revision history:
 *
...
void csminwel(double (*fcn)(double *x, int n, double **args, int *dims),
   ...
   int retcode[3], fc=0, ih, nn;
   int i;

   nn = n*n;     /* n: dimension size of x or xh */
   *itct = 0;    /* itct: Number of iterations */
   *fcount = 0;  /* fcount: Number of evaluations of the function */
   ...
   while (!done) {
      for (i=0; i<n; i++)
         ...
         printf("-----------------\n-----------------\n");
         printf("f at the beginning of new iteration, %20.10f\nx = ",f[0]);
         for (i=0; i<n; i++) {
            printf("%15.8g ",x[0][i]);
            if (i%4==3) printf("\n");
         }
...
#ifdef VERBOSE_DETOUPUT
         printf("----\nImprovement on iteration %d = %18.9f\n",(int)*itct,f[0]-*fh);
#endif
      if ((int)*itct > nit) {
#ifdef VERBOSE_WARNINGS
         printf("\nWarning: Iteration count termination\n");
#endif
         done = 1;
      }
      else if (stuck) {
...
int numgrad(double *g, double *x, int n,
   ...
   static double delta=SCALE*1.0e-5, deltai=1.0e5/SCALE;
   ...
   *retcode = 0;
   ...
   if ((gnorm < 1.e-12) && !badg)
      *retcode = 1;  /* gradient convergence */
      ...
      dx = (double *)malloc(n*sizeof(double));   //dx = calloc(n, sizeof(double));  Commented out by TZ.
