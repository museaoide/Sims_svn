bvarWrap2 <- function(x, prior) {
    ...
    sigfac <- array(0, c(nv, nv, T))
    for (isig in 1:nsig) {
        sigval <- chol(crossprod(t(A %*% diag(exp(lmd[ , isig])))))
        sigfac[ , , (Tsigbrk[isig]+1):Tsigbrk[isig+1]] <- sigval
    }
    vout <- rfvarKF(ydata = window(slimdata4, end=enddata), lags = Lags, sigfac = sigfac, prior = prior)
    lh=-sum(vout$lh)
    attr(lh,"prior") <- prior
    attr(lh,"sigfac") <- sigfac[ , , Tsigbrk[-1]]
    attr(lh, "T") <- T
