...
int eigrsym(TSdvector *eval_dv, TSdmatrix *eVec_dm, const TSdmatrix *S_dm)
{
   ...
   //   ul: if =='U' or 'u', S_dm is upper triangular; if =='L' or 'l', S_dm is lower triangular.
   //
   ...
   if ( D != S) {
      switch (ul) {
         case 'U': case 'u':
            ...
            else  fn_DisplayError(".../mathlib.c/chol():  R input square matrix must be symmetric (and positive definite)");
      ...
      else  fn_DisplayError(".../mathlib.c/chol():  When D==S, upper or lower triangular output must be consistent with upper or lower symmetric input; otherwise, use the option with D != S");
   }
...
int invrtri(TSdmatrix *X_dm, TSdmatrix *A_dm, const char un)
{
   ...
   if ( (_n != A_dm->ncols) || (_n != X_dm->nrows) || (_n != X_dm->ncols) )
      fn_DisplayError(".../mathlib.c/invrtri(): both input and output matrices (1) must be square and (2) have the same dimension");
...
int BdivA_rrect(TSdmatrix *X_dm, const TSdmatrix *B_dm, const char lr, const TSdmatrix *A_dm)
{
   ...
   //
   //Outputs:
   //  X = A\B or A/B where X is _n-by_r if \ (AX=B) or _r-by-_n if / (XA=B).
   ...
   if (lr=='/') {
      ...
      if ( _m < _n)  fn_DisplayError(".../mathlib.c/BdivA_rrect(): A_dm->nrows must be <= A_dm->ncols for / (right division).  Otherwise, take the nonsigular square part of A_dm and use BdivA_rgens()");
   }
   else if (lr=='\\') {
      ...
      if ( _m < _n)  fn_DisplayError(".../mathlib.c/BdivA_rrect(): A_dm->nrows must be >= A_dm->ncols for \\ (left division). Otherwise, take the nonsigular square part of A_dm and use BdivA_rgens()");
   }
...
int BdivA_rgens(TSdmatrix *X_dm, const TSdmatrix *B_dm, const char lr, const TSdmatrix *A_dm)
{
   ...
   //
   //Outputs:
   //  X = A\B or A/B where X is _m-by_r if \ (AX=B) or _r-by-_m if / (XA=B).
   //  Returns info (if info==0, the execution is successful; if info == -i, the ith parameter has an illegal value.)
   //--------
   //Inputs:
...
void Aldivb_spd(TSdvector *x_dv, TSdmatrix *A_dm, TSdvector *b_dv, char an) {
   //??????? Some options are NOT tested yet.
   //Output x = A\b where x_dv is an _n-by-1 vector.
   //  If x_dv->v = b_dv->v, x_dv->v will be replaced by new values.
   //--------
   //Inputs:
...
double logdeterminant(TSdmatrix *A_dm) {
   ...
   errflag = lurgen(U_dm, (TSivector *)NULL, A_dm);  //Gets only the U part of A_dm.
...
void VectorTimesSelf(TSdmatrix *C_dm, const TSdvector *a_dv, const double _alpha, const double _beta, const char ul)
{
   ...
   //Output is the symmetric matrix C and all other arguments are inputs.
...
void MatrixTimesMatrix(TSdmatrix *C_dm, TSdmatrix *A_dm, TSdmatrix *B_dm, const double _alpha, const double _beta, const char tn1, const char tn2) {
   ...
   if ( !C_dm || !A_dm || !B_dm )  fn_DisplayError(".../mathlib.c/MatrixTimesMatrix(): All input matrices must be created (memory-allocated)");
...
void PermuteColsofMatrix(TSdmatrix *A_dm, const TSivector *indx_iv)
{
   ...
   for (_j=_n-1; _j>=0; _j--)
      memcpy(A+indx[_j]*_m, B_pd+_j*_m, _m*sizeof(double));
...
void PermuteRowsofMatrix(TSdmatrix *A_dm, const TSivector *indx_iv)
{
   ...
   #if defined( INTELCMATHLIBRARY )
   for (_i=_n-1; _i>=0; _i--)
      cblas_dcopy(_m, B_pd+_i, _n, A+indx[_i], _n);
   #else  //Default to my own routine.
   _m = A_dm->ncols;
   for (_j=_m-1; _j>=0; _j--)
     for (_i=_n-1; _i>=0; _i--)
        A[mos(indx[_i], _j, _n)] = B_pd[mos(_i, _j, _n)];
   #endif
...
void PermuteMatrix(TSdmatrix *A_dm, const TSivector *indx_iv)
{
   ...
   for (_j=_n-1; _j>=0; _j--)
     for (_i=_n-1; _i>=0; _i--)
        A[mos(indx[_i], indx[_j], _n)] = B_pd[mos(_i, _j, _n)];
...
void CopyVector0(TSdvector *x1_dv, const TSdvector *x2_dv) {
   ...
   else if ( !x2_dv->flag )  fn_DisplayError(".../mathlib.c/CopyVector0(): R input vector must be given values.");
   else _n = x2_dv->n;
...
void CopySubmatrix(TSdmatrix *x1_dm, const int br1, const int bc1, TSdmatrix *x2_dm, const int br2, const int bc2, const int nrs, const int ncs) {
   ...
   if ( (bc2+ncs)<=ncols2 && (br2+nrs)<=nrows2 && (bc1+ncs)<=ncols1 && (br1+nrs)<=nrows1 ) {
      for (_j=ncs-1; _j>=0; _j--) {
         loc1 = mos(br1, bc1+_j, nrows1);      //Points to the top of the column in x1_dm.
...
void CopySubmatrix2vector(TSdvector *x1_dv, const int ptrloc1, TSdmatrix *x2_dm, const int br, const int bc, const int nels) {
   //Ouputs:
   //  x1_dv, whose elements from x1_dv->v+ptrloc1 to x1_dv->v+ptrloc1+nels-1 are copied from a column of x2_dm->M.
...
void CopySubmatrix2vector_row(TSdvector *x1_dv, const int ptrloc1, TSdmatrix *x2_dm, const int br, const int bc, const int nels) {
   ...
   if ( (bc+nels)<=x2_dm->ncols && (ptrloc1+nels)<=x1_dv->n) {
      #if defined (SWITCHTOINTELCMATH)             // define: use Intek MKL LAPACK library; undef: use others.
         cblas_dcopy(nels, M+mos(br, bc, nrows), nrows, v, 1);  //mos(): inclusive pointer location for x2_dm where the copy begins.
...
void CopySubvector2matrix_unr(TSdmatrix *x1_dm, const int br, const int bc, const TSdvector *x2_dv, const int ptrloc2, const int nels) {
   ...
   //          that the bc+1 column may start before the specified br.
   //  Copied elements (x1_dm->M+ptrloc1 to x1_dv->M+ptrloc1+nels-1) from x2_dv->v.
   //  Always sets x1_dm->flag = M_GE after the call to this function.
   //Inputs:
   //  Copying elements from x2_dv->v+ptrloc2 to x2_dv->v+ptrloc2+nels-1 (to x1_dm->M).
...
void TransposeSquare(TSdmatrix *B_dm, TSdmatrix *A_dm) {
   ...
   if ( ((Aflag=A_dm->flag) & M_SU) && (Aflag & M_SL) )  fn_DisplayError(".../mathlib.c/TransposeSquare():  Matrix is already both SU and SL, so there is no need to transpose.  Check a possible bug in your program");
   ...
   if ( (A=A_dm->M) != (B=B_dm->M) ) {
      ...
      if ( (Aflag & M_GE) && (Aflag & M_UT) ) {
         ...
         for (_j=0; _j<_n; _j++)
            for (_i=_j+1; _i<_n; _i++) {
               //Off-diagonal elements.
               swap(A[mos(_i,_j,_n)], A[mos(_j,_i,_n)], tmpd);
//               tmpd =  A[_j*_n+_i];
...
void diagdv(TSdvector *x_dv, TSdmatrix *x_dm)
{
   ...
   //=== Checking dimensions and memory allocation.
   if ( !x_dv || !x_dm ) fn_DisplayError(".../mathlib.c/diagdv(): Both the input vector and matrix must be created (memory-allocated)");
   else {
...
void ergodicp(TSdvector *p_dv, TSdmatrix *P_dm) {
   ...
   //------------
   //Inputs:
   //  P_dm: n-by-n Markovian transition matrix.
