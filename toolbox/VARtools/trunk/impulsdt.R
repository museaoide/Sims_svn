impulsdt <- function (A,nstep) {
  ...
  if (min(svdA0$d)/max(svdA0$d) < 1e-20) {
    ...
    Aplus <- A[,,2:nlag]
    Aplus <- Aplus[,,seq(from(nlag-1), to=1, by=-1)] # Reverse the time index to allow simple matrix mult for irf
    Aplus <- aperm(Aplus,c(1,3,2))
    dim(Aplus) <- c(dim(Aplus)[1] , dim(Aplus)[2] *  dim(Aplus)[3])
    Aplus <- A0i %*% Aplus
    dim(Aplus) <- c(dim(Aplus)[1] * dim(Aplus)[2],  dim(Aplus)[3])
    irhs <- 1:(nlag*nvar)
