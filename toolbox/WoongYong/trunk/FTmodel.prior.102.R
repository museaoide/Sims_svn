...
#         N(mu,sd)
#         log(real C) = 7.634 at 1954Q3

prior.param$cbar <- c(mu=7.634,sd=.1)
...
prior.param$sig2m <- c(shape=2,rate=1e+5)
...
prior.param$sig2r <- c(shape=2,rate=1e+5)
                        
# sig2pc  (0,inf) variance of PC shock
#         IG(shape,rate)
#         rate in IG = scale in Gamma

prior.param$sig2pc <- c(shape=2,rate=1e+5)
