PstDnsty7 <- function(param,SysEq,z,bp0,trf,dfactor=1,div,prior.param){
# Input
...
#
  ...
  #---------- when not
	  ...
	  if (any(param[5]<0,param[6]<0,param[7]<0,param[8]<0,param[9]<0,param[9]>1,param[10]<0,param[14]<0,param[15]<0,param[16]<0,param[17]<0,param[18]<0,param[19]<0,param[20]<0,param[20]>1,param[21]<0)) {
	    ...
	    fout <- list()
	    fout$pnlty$pnlty <- 0
	    return(fv=list(lkh=0,prd=-dfactor*Inf,eu=c(-9,-9),pnlty=fout$pnlty))
	  }
 }  
  ...
  param["rbar"] <- param["abar"] <- param["gamm"] + param["thet"] + param["pibar"]
    ...
    fout <- list()
    fout$pnlty$pnlty <- 0
		return(list(lkh=0,prd=-dfactor*Inf,eu=c(-8,-8),pnlty=fout$pnlty))
	}
  ...
  fout <- SolveGensys(param=param,SysEq=SysEq,bp0,div=div)

  #---------- existence/uniqueness
  if (fout$eu[1] != 1 || fout$eu[2] != 1 || fout$pnlty$pnlty > 0)	{
  	cat("non existence or non uniqueness: ",fout$eu," or penalty:",fout$pnlty$pnlty,"\n")
  	return(fv=list(lkh=0,prd=-dfactor*Inf,eu=fout$eu,fout=fout,pnlty=fout$pnlty))
  }
  #if (fout$eu[1] != 1 || fout$eu[2] != 1) return(fv=-dfactor*Inf)
...
#  shat.old <- c(1,fout$SSvars)
#  shat.old["b"] <- bp0[1]/param["abar"]
#  sig.old <- diag(c(0,r=.25*1e-4,a=.25*1e-4,p=.01,w=1e-4,c=1e-2,cc=1e-4,b=1,tau=.1,lam=1,xir=.5*param["sig2r"]/param["rhor"],xipc=.5*param["sig2pc"]/param["rhopc"]))
  ...
  mu0["b"] <- fout$SSvars["b"]
  mu0["c"] <- fout$SSvars["c"]
  mu0["tau"] <- fout$SSvars["tau"]
  #mu0[2:12] <- fout$SSvars
  Sig0 <- diag(10,nrow=12)
  Sig0[1,1] <- 0
...
shat.vec <- array(0,c(13,1,nz))
sig.vec  <- array(0,c(13,13,nz))
yhat.vec <- array(0,c(5,1,nz))
lkh.vec   <- matrix(0,nrow=nz,ncol=1)
  ...
  for (indx in 1:nz){
    ...
    shat.vec[,,indx] <- shat.old
    sig.vec[,,indx]  <- sig.old
    yhat.vec[,,indx] <- z[indx,] - kfout$fcsterr
    yhat.vec[3,,indx] <- yhat.vec[3,,indx] + param["gamm"]*indx
    yhat.vec[4,,indx] <- yhat.vec[4,,indx]*exp(param["gamm"]*indx)
    yhat.vec[5,,indx] <- yhat.vec[5,,indx]*exp(param["gamm"]*indx)
    lkh.vec[indx]   <- kfout$lh[1] + kfout$lh[2]
  }
  ...
  return(list(lkh=dfactor*lkh,prd=dfactor*prd,eu=fout$eu,pnlty=fout$pnlty,shat.vec=shat.vec,sig.vec=sig.vec,yhat.vec=yhat.vec,lkh.vec=lkh.vec,MMdiag=diag(MM)))
} ## THE END