expv <- function ( t=1, A=diag(-.1,nrow=3,ncol=3), v=c(1,00), tol=1e-7, m=30 ){
  ## This routine is aimed mainly at handling large sparse matrices.  This file is ony partially converted to R.  CAS 6/05/2005
  ##-------------------------------
...
##    for (j in  1:n){
##        for (i  in 1:n){
##            if (runif(1) < 0.5) A(i,j)  <-  0
##        }
##    }
##    v  <-  diag(1,nrow=n,ncol=1)
##    A  <-  sparse(A); ## invaluable for a large and sparse matrix.
##    >>>> at this point, stopped converting this example.  There are, in the Matrix package,
##         sparse matrix types and methods, but I don't need this right now.  (CAS 6/5/2005)
##    tic
##    [w,err]  <-  expv(1,A,v);
##    toc
##
