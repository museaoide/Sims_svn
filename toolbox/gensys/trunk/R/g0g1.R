g0g1 <- function(ex,x,xl,shock,experr,param){
  ...
  names(Phi) <- names(ex)
  names(Pi) <- names(ex)
  ## What is returned is a list of 4 lists of nf functions.  The values the functions returns
  ## are those of the expressions in ex, but the values also have "gradient" attributes, so
  ## that attr(function-called-with-non-symbolic-arguments,"gradient") is the gradient vector,
